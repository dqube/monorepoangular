...
import { ISyncDialogOpenResult, IDrawerOptions } from './drawer.types';
...
export class SyncDialogService {
  ...
  open({
    ...
    bodyScrollable = true,
    showAnimation = true
  }: IDrawerOptions): ISyncDialogOpenResult {
    ...
    const drawerRef = this.overlayContainerRef.createComponent(
      componentFactoryResolver_.resolveComponentFactory(SyncDialogComponent),
      injector
    );
